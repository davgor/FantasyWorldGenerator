...
def found_cities(seed, parents, owners, quotas, candidates, scores, distance, spacing, world_reach, survivors=(), forbidden=(), years_per_round=250, origin_min_separation_degrees=90, start_year=0, diaspora_interval_years=1000, diaspora_bonus_per_parent=1, food_allowances=None, city_limit=24, diaspora_bonus_used=(), used_civilizations=(), magic_enabled=True, minimum_residents=None):
    ...
    if food_allowances is not None:
        quotas={key:min(value,food_allowances.get(key,0)//(minimum_residents or {}).get(key,40)) for key,value in quotas.items()}
    ...
    return sites,dict(version=4,minimum_residents={key:(minimum_residents or {}).get(key,40) for key in owners},effective_quotas=quotas,diaspora_interval_years=diaspora_interval_years,diaspora_bonus_used=sorted(bonus_used),used_civilizations=sorted(ever_used),start_year=year_offset,end_year=year_offset+(turn-1)*years_per_round,years_per_round=years_per_round,elapsed_years=(turn-1)*years_per_round,origin_min_separation_degrees=origin_min_separation_degrees,turns=turn,target_cities=target,placed_cities=len(sites),stop_reason=stop,events=events,
