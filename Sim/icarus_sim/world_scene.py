...
def build_scene(world):
    ...
    scene={'version':1,'unit':'metres','coordinates':'globe-centred XYZ; Y north; X at latitude 0 longitude 0; Z at latitude 0 longitude 90',
           ...
           'limits':'Regional routes remain coarse transport proposals, including unengineered bridge candidates; local connection failures are explicit.'}
    gates={}
    for city in world['city_plans']['cities']:
        site=next(s for s in world['settlements']['sites'] if s.get('uid',str(s['id']))==city['city_uid'])
        f=frame(world,site)
        for i,path in enumerate(city.get('street_paths_m',[])):
            scene['streets'].append({'id':f'{city["city_uid"]}:street:{i}','city_uid':city['city_uid'],
                'positions_m':[position(field,local_direction(f,*p)) for p in path]})
        for c in city.get('road_connections',[]):
            gates[(c['route_index'],c['end'])]=c
            pos=position(field,c['direction'])
            scene['junctions'].append({'id':c['junction_id'],'city_uid':city['city_uid'],'route_id':c['route_id'],
                                      'position_m':pos,'status':c['status']})
            if c['status']=='connected':
                vertices=[position(field,local_direction(f,*p)) for p in c['local_path_m']]
                vertices[-1]=pos
                scene['streets'].append({'id':c['junction_id']+':approach','city_uid':city['city_uid'],
        ...
        for b in city['plots']:
            ...
            scene['buildings'].append({'id':f'{city["city_uid"]}:{b["id"]}','city_uid':city['city_uid'],
