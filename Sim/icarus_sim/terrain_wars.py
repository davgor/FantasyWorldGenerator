...
def contested_pairs(cities, cores, routes, radius, spacing, proximity_factor=PROXIMITY_FACTOR):
    ...
    for i, a in enumerate(cities):
        for j in range(i + 1, len(cities)):
            ...
            pairs.append({'a': i, 'b': j, 'a_uid': a['uid'], 'b_uid': b['uid'],
                          ...
                          'supply_pressure': supply, 'pressure': min(1., proximity + supply),
                          'kind': war_kind(a, b)})
...
def resolve_wars(cities, cores, routes, layers, radius, spacing, seed, age,
    ...
    `rolls` replaces the seeded draw in tests, keyed by war id with `'*'` as a default.
    """
    by_site = {core['site_id']: core for core in cores}
    wars, fates = [], {}
    for pair in contested_pairs(cities, cores, routes, radius, spacing):
        a, b = cities[pair['a']], cities[pair['b']]
        if a['uid'] in fates or b['uid'] in fates:
            continue
