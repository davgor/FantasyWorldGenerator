"""Sphere-grid geometry: the generator's own node layout, as pure arithmetic.

A world is laid out as ``sphere_grid(n, radius)``: one node at each pole and a row-major
body in between, with the seam column dropped because it duplicates column zero.
...
only ever reads placed records needs none of this. A reader that must *choose* ground has to
address cells the world never occupied, which means reproducing the indexing exactly.

These functions are not a re-derivation. They are the generator's arithmetic, in the
generator's order, and they must stay that way: float addition is not associative, so
rewriting a sum here moves a world. ``Sim/tests/test_world_geometry.py`` holds them to the
generator at five raster sizes; it is the only pin, and the generator is the oracle.

No filesystem, no network, no engine, and nothing here imports a generator or a reader.
"""
import math
