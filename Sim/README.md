...
Run `python tools/validate_repo.py` from the standalone repository root. Launch the browser lab with `python tools/terrain_lab.py --serve`. The default is a full tectonic globe with Random and Parameters modes. See [layered world documentation](../docs/terrain-world-layers.md) for recipes, aquatic habitats, independent leylines, sky surfaces and supply diagnostics.
