...
The lab now defaults to recipe **3**, generation algorithm **16**, with sixteen stages. Launch with `python tools/terrain_lab.py --serve`; the page opens on the committed sample world and **Run simulation** builds a new one, timed. `--phase` stops computation at a stage for `/generate` and for `--fresh`, not for the served sample, which is always a completed world. Previous/Next inspects saved states without regeneration:
...
Existing worlds are disposable until the first player-facing consumer ships; see that record.
[023-world-compatibility-policy.md](decisions/023-world-compatibility-policy.md)
...
Algorithm-8 worlds require regeneration. Settlement schema 10 and civilization report 1 expose entity identity and city class; rural report 9 carries architecture style keys, `civilization_region` and the route-defence demand independently of seed-local interaction groups. Report 9 also stops a coastal harbour being founded on a node a fortress already holds: the harbour pass excluded cities and hamlets from its candidate nodes but not forts, so 28 of 56 forts in a seed-42 size-33 world shared their exact node with a hamlet. Age advancement refuses a report older than 9.

Mixed populations select independent Maritime humans, Desert humans, Cold peoples, Large islanders, Deep rainforest humans and Heartland humans alongside dwarves, elves, gnomes and coastal Tidekin. The generic human/highland/woodland profile IDs are retired. Each nonempty civilization has exactly one suitability-ranked capital; other cities are medium at suitability >= 0.65 and small otherwise. Zero-city civilizations are valid. See [standalone civilizations](decisions/016-standalone-civilizations.md) for habitat priority, independent entity definitions, cultural region layers and compatibility. Environmental preferences differ from cultural traits: maritime, islander, industrial and pirate. Islander status requires a small connected landmass; any eligible people may adopt it. "Small" means a connected land component holding under 15% of this world's total land area — a relative test with no absolute metre floor. The former 3 km² floor was authored for the 11.15 km reference world. On the 200 km world the smallest sphere-grid cell is 15.217 km² at size 17, so at the default raster no component of any seed could reach that floor and island_habitat was identically zero. It was not zero at every raster — cells fall to 1.914 km² at size 33 and 0.240 km² at size 65, where a one-cell polar component does clear 3 km² (seed 40 reads 1 island cell at size 33 and 6 at size 65 under the old rule) — but it was close enough to zero that in practice no people could take the trait and no island site could place. Tidekin do not live underwater in this version.
...
From stage 13 the world exports two independent habitat passes: `wildlife` holds animal hunting grounds and `beast_nests` holds monster territory. All 680 catalogue entries (311 animals, 369 monsters) have structured profiles in `Sim/icarus_sim/terrain_nest_profiles.json`, each carrying a stable species ID, a `class` of `animal` or `monster`, a danger `tier`, habitat medium, temperature bounds, required fields, weighted preferences, occurrence and spacing. Animals additionally carry a feeding `role`; the 21 roles are described once in the same file and supply the biome table every animal of that role uses — all 311 animals take their role’s table and not one overrides it. Monsters carry no role at all: every one of the 369 now carries its own per-species `biome_weights`, so monster biome steering is authored species by species rather than role by role, and each monster also carries a `likeness`. Counts re-measured against `terrain_nest_profiles.json` on 2026-09-20. Profiles are artistic first-pass parameters, not biological validation.
...
The two passes differ only in what refuses a candidate. **An animal competes with its own species only** — a wolf range over a deer range is the point of the map, not a collision. **A monster is refused only by a monster of its own tier**, so a lesser lair nests inside a greater territory and hunting grounds overlap by construction. The rule read *equal or greater* until 2026-09-21, which says the opposite in one direction: a goblin drawn after a dragon was refused by it, so a lesser lair nested inside a greater territory only when the draw happened to reach it first. It also refused the pyramid — a tier-one candidate was held off by every lair on the world while a tier five was held off only by other tier fives. Each anchor exports a centre and a `range_m` (half the authored spacing); `den` marks the ones that are a findable place to raid rather than only a range where the creature is met.
