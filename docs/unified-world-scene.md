...
Final generation and subsequent completed age transitions include `world_scene` version 1 alongside `city_plans`, `hamlet_plans` and `castle_plans`. It contains globally positioned building footprints and transforms, street polylines, regional-road polylines and junctions. Entries are tagged with `settlement_kind` (`city`, `hamlet` or `castle`) plus `city_uid`, `hamlet_id` or `fortress_id` so consumers can filter independently. XYZ is globe-centred in metres: Y points north, X passes through latitude 0/longitude 0, Z through latitude 0/longitude 90. Surface positions use `(radius_m + canonical_height_m) * direction`.
...
The lab globe includes global street/road polylines and building footprints. Ruined former cities draw last as a broken-keep icon with a gold outline and “RUIN” label, larger than live city houses, so they stay readable over streets and terrain. The city inspector shows the same local street connections (orange approach ends), natural biome colors and magic outlines. Engine streaming, collision and asset realization remain adapter responsibilities. Convert metres to Unreal centimetres only at that boundary.
