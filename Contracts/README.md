...
The current world envelope is schema **2**, recipe **3**, generation algorithm **12**, phase schema **2**, magic schema **3**, terrain schema **6**, and history schema **1**. Population profiles and building-pack inputs use schema **3**; settlements use schema **13**, civilization reports use schema **2**, and rural reports use schema **6**. Algorithm-8/9/10/11 worlds and retired generic human profile IDs require regeneration. Recipes 1/2 and old age/save contracts are rejected and must be regenerated. Natural IDs are sparse, never list offsets; retired IDs 9/10/11/12/14 are invalid. The 104 magical states use explicit `core.school` identities. See [the canonical behavior contract](../docs/terrain-world-layers.md).
