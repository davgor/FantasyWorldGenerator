...
> This card's own instruction is **"delete it or leave it, but do not connect it."** Leaving it is the resulting outcome. **“Do not connect it” is a standing constraint, not a closed
> instruction:** `add_magic` stays unreachable and `generate_networks` stays the sole writer, and the
> `shadow_note` in `docs/conformance/version-bindings.json` states outright that if anything ever wires
> `add_magic` into a generation path that binding is wrong and the two writers need arbitration first.
> Retiring this card does **not** retire that constraint.
> Nothing here is a blocker. Read it for what it measured, not for what it instructs.

> **Re-tested 2026-09-21 against the tree — CONFIRMED, claim reproduces.** `terrain_leyline_history.py:11` imports `add_magic` and never calls it. Still unreachable.
