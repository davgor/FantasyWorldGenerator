...
> **RETIRED 2026-09-21 — my oversight, caught by the sweep lane, not deliberate.** I kept this in the
> backlog when I retired the other port cards because its state line reads “harness fixed … a short,
> classified residual” and I filed it as bookkeeping without checking what the residual was. It is
> native: every item still red is a `tests.test_native_world` parity failure — five `delivered_food`
> and three `irrigation_benefit` at last-ulp, one `dominant_magic` over 33 of 1089 cells, one
> `lunar_sensitivity`. Under [027](../../docs/decisions/027-native-port-deferred-to-a-full-redo.md)
> none of that will be chased against this tree.
>
> **Its one non-native item is already closed.** The card records two tests failing on
> `asset_list_sha256` because a peer lane edited `asset_list.py` after the registry was generated,
> noting “regenerating the registry closes both; no C++ is implicated.” Checked 2026-09-21:
> `python tools/build_asset_registry.py --check` exits 0 at 1707 bindings. Nothing non-native is left.
>
> **What is worth keeping from it:** the finding that the native core is bit-exact with the reference
> at 289/289 cells, `maxdiff` exactly 0, and that the ninety-failure “red at baseline” story was an
> artifact of the suite comparing two different worlds rather than a code defect. Whoever scopes the
> port should read that before re-deriving it, and should not requote the old failure counts.
