...
>
> The card asked for one kept-current number saying how much of the Python reference the native path reproduces. With the port deferred wholesale that number is knowingly zero-and-frozen, so a ledger tracking it measures nothing that moves. **The non-native half of its complaint survives**: the board's “complete” should still say which layer it is complete in. That is now handled by the board README stating plainly that every capability is Python-only.
