...
> **CLOSED 2026-09-21.** Both halves of the fix landed — `terrain_ruins.source_school` and `Core/legacy.cpp`/`legacy.hpp` with the parameter spelled out at both `Core/ages.cpp` call sites and no default argument. The only outstanding item was the native comparison exercising a villain cause with a non-null school, which needs a compile. With the native port deferred to a full redo ([027](../../docs/decisions/027-native-port-deferred-to-a-full-redo.md)) that test will not be written against this tree, and the `Core/` edits it would have covered will be rewritten. Note for whoever scopes the port: `WorldConfig::villain_rise` is `0.`, so nothing can seat a villain natively and the branch is unreachable in any case.
