...
- The age band reports rather than raises: a cost estimate with its basis and a `measured`
  flag, plus `commit` to actually advance in chunks of ten. Whatever `validate_age_world`
  refuses is returned as a `blocked` document rather than restated here, so the real gate
  is untouched and cannot be misquoted. **A size-513 world is age-able** — the ceiling is
  1025, not 257, and the span is also capped at 50 ages by this module.
  A span that is not a whole number of ages ticks its remainder.
- `world_clock`, `quests` (lifecycle only, no difficulty), `time_advance`, and a
  `history.operations` row per call **that advances the world** — a sub-day span crossing
  nothing moves the clock and records nothing. `POST /world/advance-time` in the lab, with a
