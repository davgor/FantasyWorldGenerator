...
> **CLOSED 2026-09-21 — acceptance (2) delivered as `tests/test_ceiling_sentinels.py`, with
> one instance still armed and handed back to the coordinator. Read the residue below
> before treating this as finished.**
>
> **The survey the card said it had not run has now been run, and the answer is sixteen,
> not three.** Every numeric literal bound to a control name, in every module the three
> unittest discoveries pick up, checked against every control the registry declares a
> domain for — the whole legal range, not a sweep near `size`. Sixteen literals in nine
> modules sit exactly one step outside a published bound. The card's "not established: that
> these are the only three instances" was right to be cautious and the true figure is five
> times larger.
>
> **Fifteen of the sixteen cost nothing if their bound moves**, because they are handed
> straight to a constructor that refuses before any work happens, so a newly-legal value
> builds a `Config` and the test fails loudly on the spot. One does not: `size=1026` at
> `Sim/tests/test_terrain_patch.py:61` reaches `patch_request`, which generates. That
> distinction is the actual harm and is recorded per entry as `cost`, because the pattern
> alone does not tell you which instance matters.
>
> **The shape of the check, and the alternative rejected.** The card's literal acceptance
> (2) — "no rejection sentinel in a test is a literal equal to a published bound plus one"
> — is mechanically decidable and would be **red today**, on an instance in a
> provenance-pinned file that this lane may not edit. A permanently red suite entry teaches
> people to ignore red. What landed instead is a census with a ratchet, in both directions:
> a fragile sentinel that is **not** recorded fails on arrival, so the pattern cannot
> spread; and a recorded sentinel that stops being one step outside its bound fails naming
> the file, which happens on exactly the day the bound moves and the sentinel silently
> becomes legal work. That is the moment the card exists to catch, and it is caught at that
> moment rather than an hour later when someone notices the suite is slow.
>
> Scoped honestly: a sentinel written `registry(3)['size']['max'] + 1` is the repair, not
> the defect, and is not flagged — only bare literals are read. The scan is module-wide
> rather than scoped to the `assertRaises` block, because the armed instance sits in a tuple
> of request bodies iterated by a `for`, several lines above the `with` that consumes it;
> a `with`-scoped scan misses the one instance the card is about, which was measured before
> the design was chosen rather than assumed.
>
> **Residue, owed to the coordinator and not done here.** The one-line replacement at
> `Sim/tests/test_terrain_patch.py:61` —
> `registry(3)['size']['max'] + 1` in place of the literal `1026` — still has not been
> applied. That file is provenance-pinned (`status: modified`, last revised 2026-09-20), so
> the edit needs a revision row in `provenance/extraction-manifest.json`, which this lane is
> forbidden to write. It is recorded as the first entry of `SENTINELS`, with its blocker,
> and deleting that entry without deriving the literal fails the test.
>
> Documentation impact: **already done, and the card says so itself.** The comment at
> `tests/test_native_genesis.py:83-92` now names containment rather than the wrong reason;
> re-read on 2026-09-21 and confirmed correct. No edit was made.

Owner: none. State: **closed, with one instance armed and handed to the coordinator.**
