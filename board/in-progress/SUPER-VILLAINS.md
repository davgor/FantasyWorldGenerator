...
Known limitations: appending four schools **retires every saved world** at `terrain_history.py:642` (the guard now reads `Expected exactly {len(SCHOOLS)} networks: the {len(KNOWN_SCHOOLS)} known schools and the {len(SCHOOLS)-len(KNOWN_SCHOOLS)} hidden` — this card previously quoted `Expected exactly eight networks`, which the hidden-schools work replaced and which now exists nowhere in the tree) — unavoidable, so it is done once deliberately with `history.version` bumped in the same slice. `PARITY_SEED=42, PARITY_SIZE=33` proves one seed did not move, not that no seed moved; diff `tools/export_showcase.py` across seeds 42/73/108 by hand. `Core/settlements.cpp:214` indexes `school_names()[dominant_magic[z][x]]` without a bounds check. Blood and Eldritch read one rebuild behind, since `evaluate_networks` runs before `civilization()` and `add_nests` in both rebuild paths.
